...
#왼쪽 레이아웃에 다마고치 설명 페이지
#오른쪽 레이아웃 위쪽에 gif 아래에 게이지
...
PROCESS01_PATH='C:/Streamlit/PlanTOUCH/process01.png'
PROCESS02_PATH='C:/Streamlit/PlanTOUCH/process02.png'
PROCESS03_PATH='C:/Streamlit/PlanTOUCH/process03.png'
PROCESS04_PATH='C:/Streamlit/PlanTOUCH/process04.png'

PROCESS01=get_image_base64(PROCESS01_PATH)
PROCESS02=get_image_base64(PROCESS02_PATH)
PROCESS03=get_image_base64(PROCESS03_PATH)
PROCESS04=get_image_base64(PROCESS04_PATH)
...
show_gif = True
if st.button("Change Gif"):
    show_gif = not show_gif
current_gif = FLOWER_GIF1 if show_gif else FLOWER_GIF0
current_process = 1

# 각 프로세스에 대한 이미지 경로 (Base64로 인코딩된 문자열)
PROCESS_IMAGES = {
    1:  PROCESS01,
    2:  PROCESS02,
    3:  PROCESS03,
    4:  PROCESS04
     ...
     # 버튼을 눌렀을 때의 동
if st.button("Change Process", key="change_process_button"):
    current_process += 1

    # current_process 값이 5 이상이면 1로 리셋
    if current_process > 4:
        current_process = 1

# 현재 보여줄 프로세스 이미지 선택
process_png = PROCESS_IMAGES[current_process] 
...
with game_zone:   
     game_zone.markdown(f""" <div class = 'info_message'> '{st.session_state.user_name}'님의 '{st.session_state.plant_name}🍀'</div> 
                        """,unsafe_allow_html=True)
     game_zone.markdown(f""" <div class = 'info_message'> {current_process}'</div> 
                        """,unsafe_allow_html=True)
    
    #  game_zone.markdown(f"""  <p style=  "padding-left : 18rem";>
    #                           <img src="data:img/logo_char.jpg;base64,{FLOWER_GIF0}"  width="350px"; height="350px";/><p/>
    #                           """, 
    #                           unsafe_allow_html=True )
     game_zone.markdown(f'<img src="data:image/gif;base64,{current_gif}" style="width: 350px; height: 350px;"/>', unsafe_allow_html=True)
     ...
     st.markdown(f'''
                 ...
                 ''', unsafe_allow_html=True)   
     # 버튼을 눌렀을 때의 동
     if st.button("Change Process"):
         current_process = (current_process % 4) + 1

# 현재 보여줄 프로세스 이미지 선택
     process_png = PROCESS_IMAGES[current_process]   
     game_zone.markdown(f'<img src="data:image/gif;base64,{process_png}" style="width: 700px; height: 40px;"/>', unsafe_allow_html=True)
...
a = st.empty()
b = f"<img src='data:image/gif;base64,{process_png}' style='width: 700px; height: 40px;'/>"
while(True):
...
#         percentage1+=0.5 
#         st.markdown(f'''<div class="meter green2 nostripes">
#         <span style="width: {percentage1 * 100}%"></span>
#     </div>
# ''', unsafe_allow_html=True)
